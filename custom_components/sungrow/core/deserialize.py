...
def _decode_int_signal(
    ...
    if int_value == signal.na_value:
        int_value = 0
    elif signal.base_datatype == "S16" and int_value > 0x7FFF:
        int_value -= 0x10000
...
def _decode_utf8_signal(signal: SungrowSignalDefinition, raw: list[list[int]]):
    value = "".join([chr(c[0] >> 8) + chr(c[0] & 0xFF) for c in raw]).strip("\x00")

    return value


def _decode_base_signal(signal: SungrowSignalDefinition, raw_value: list[int]):
...
def _decode_array_signal(signal: SungrowSignalDefinition, raw_value: list[list[int]]):
...
def _decode_signal(
    ...
    if signal.array_length == 1:
        return _decode_base_signal(signal, raw_value)  # type: ignore
    else:
        return _decode_array_signal(signal, raw_value)  # type: ignore
